...
void* MemoryHeap_malloc(MemoryHeap* heap,size_t size)
{
	...
	if(heap->headNode==NULL){
		if(realSize > ((uint8_t*)heap->stopAddr)-((uint8_t*)heap->startAddr)){
			return NULL;
		}
...
static MemoryHeapNode* getAllocableProNode(MemoryHeap* heap,size_t realSize)
{
	if(heap->curNode==NULL){
		heap->curNode = heap->headNode;
	}
	//from cur to end.
	for(MemoryHeapNode* node=heap->curNode ; node!=NULL ; node=node->nextNode){
		if(canAllocAtNext(heap,realSize,node)){
			return node;
		}
	}
	//from frist to cur.
...
size_t MemoryHeap_real_used(MemoryHeap* heap)
{
	assert((heap!=NULL));
	size_t used = 0;
	for(MemoryHeapNode* node=heap->headNode ; node!=NULL ; node=node->nextNode){
		used+=node->realSize;
	}
	return used;
}
