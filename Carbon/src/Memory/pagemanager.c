...
uint32_t CreatPageManager(PageManager *p, uint32_t physicalmemorysize, uint32_t kernelbeginaddress, uint32_t kernelendaddress, ...) {
	...
	for (tmp_begin = (uint32_t *)args,va_args(args, uint32_t),tmp_end = (uint32_t *)args;*tmp_end != 0;va_args(args, uint32_t),tmp_begin = (uint32_t *)args,va_args(args, uint32_t),tmp_end = (uint32_t *)args) {
        uint32_t tmp;
